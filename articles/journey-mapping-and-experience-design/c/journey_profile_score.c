/*
Low-level journey profile scoring utilities.
Compile: cc journey_profile_score.c -o journey_profile_score
...
double journey_profile(Journey j) {
...
double redesign_need(Journey j) {
...
int main(void) {
    ...
    int count = sizeof(journeys) / sizeof(journeys[0]);

    for (int i = 0; i < count; i++) {
        printf("%s | journey profile %.3f | redesign need %.3f\n",
               journeys[i].id,
               journey_profile(journeys[i]),
               redesign_need(journeys[i]));
    }
