...
if command -v Rscript >/dev/null 2>&1; then
  echo "Running R assumption risk review..."
  Rscript r/assumption_risk_review.R
else
  echo "Rscript not found. Skipping R workflow."
fi

if command -v julia >/dev/null 2>&1; then
  echo "Running Julia assumption sensitivity model..."
  julia julia/assumption_sensitivity.jl
else
...
if command -v sqlite3 >/dev/null 2>&1; then
  ...
  sqlite3 outputs/tables/assumption_mapping.sqlite < sql/schema.sql
else
