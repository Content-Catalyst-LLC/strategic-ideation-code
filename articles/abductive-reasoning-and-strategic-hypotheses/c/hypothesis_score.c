/*
Low-level strategic hypothesis score utilities.
Compile: cc hypothesis_score.c -o hypothesis_score
...
double hypothesis_score(Hypothesis h) {
...
int main(void) {
    ...
    int count = sizeof(hypotheses) / sizeof(hypotheses[0]);

    for (int i = 0; i < count; i++) {
        printf("%s | %.3f\n", hypotheses[i].id, hypothesis_score(hypotheses[i]));
    }
